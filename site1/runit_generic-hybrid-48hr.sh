...
./Utils/completedGC.sh 10 ~/tim/`date +'%y%m%d-%H%M'`_completedGC > /dev/null
...
./runIOworkload-site1.sh hybridSS-48hr.xml
...
#if [[ $cephadmshell != "true" ]]; then
#    echo " " | mail -s "RGWtest site1 hybrid-48hr (${jobId}) complete" <email.address>
#fi
