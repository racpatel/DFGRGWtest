...
# reset site1 sync counters
if [[ $multisite == "true" ]]; then
    if [[ $syncPolling == "true" ]]; then
        echo "" >> $log
        updatelog "Resetting data-sync-from-site2 counters on site1 RGWs" $log
        case $CEPHVER in
            luminous)
                for rgw in $RGWhosts2 ; do
                    ssh ${rgw} 'ceph daemon `ls /var/run/ceph/ceph-client.rgw*.asok|tail -1` perf reset data-sync-from-site2' >> $log
                done
                ;;
            nautilus)
                for rgw in $RGWhosts2 ; do
            ...
            pacific)
                ...
                for rgw in $RGWhosts2 ; do
                    ssh ${rgw} "cd /var/run/ceph/$fsid2 && ceph --admin-daemon ceph-client.rgw.rgws.*.asok perf reset data-sync-from-site2"
                done
                ;;
            quincy)
                fsid2=`ssh $RGWhostname2 "ceph status |grep id:" |awk '{print$2}'`
...
# get bucket stats
    ...
    if [[ $syncPolling == "true" ]]; then
        ...
        echo "" >> $log
        updatelog "site1 sync counters:  ${site2sync}" $log
#        cmdStart=$SECONDS
#        get_dataLog
...
#        updatelog "${dataLog}" $DATALOG
        get_SyncStats
        echo -en "\nCeph Client I/O\nsite1: " >> $log
        updatelog "site1:  ${site2io}" $log
        echo -n "site2: " >> $log
        updatelog "site2:  ${site1io}" $log
    fi
...
#    for i in `rados lspools` ; do echo -ne $i"\t" >> $log ; ceph osd pool get $i pg_num >> $log ; done
    ...
    updatelog "\n osd_memory_targets: ${targets}" $log
...
#    echo -e "\n`date +%y%m%d-%H%M`" >> vmallocinfo_${stamp}.log
#    sort -gr -k2 /proc/vmallocinfo |head -50 >> vmallocinfo_${stamp}.log

    # one time check on RGW connections &&& remove later
